...
export function createColumns({ handleEdit, handleDelete }) {
  return [
    ...
    {
      title: User.GENDER_LABEL, key: 'gender', width: 80, sorter: true,
      render: ({ gender }) => gender === null ? null : h(
        NTag,
        { type: Gender.Male === gender ? 'success' : 'info' },
        { default: () => Gender[gender] }
      )
    },
    {
      title: User.STATUS_LABEL, key: 'status', width: 80, sorter: true,
      render: ({ status }) => status === null ? null : h(
        NTag,
        { type: UsingStatus.Enabled === status ? 'success' : 'error' },
        { default: () => UsingStatus[status] }
      )
    },
    { title: User.CREATE_TIME_LABEL, key: 'createTime', width: 180, sorter: true, render: (row) => row.createTimeString },
    {
